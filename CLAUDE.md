...
The owner-facing authoring and session management is built, and so is the participant-facing live voting experience: a public join page (by slug, with a QR code), single-choice voting, presence, and presenter-side live results. See [docs/live-voting-sessions.md](docs/live-voting-sessions.md). Deferred for later: enforcing the access code for public sessions, participant-visible results, and quiz scoring.
